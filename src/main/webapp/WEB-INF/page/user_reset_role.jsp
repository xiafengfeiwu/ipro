...
<body class="gray-bg">
	<div class="wrapper wrapper-content animated fadeIn">
		<c:if test="${message!=null && !''.equals(message) }">
			...
			<script>
				setTimeout(function() {
					$("#alertMsgBox").alert('close')
				}, 1500)
			</script>
		</c:if>
		<div class="row">
			<div class="col-sm-12">
				<form method="post" action="user/resetUserRoleAction.jspx" class="form-horizontal">
					<input type="hidden" name="userId" class="form-control" value="${userId }">
					<div class="form-group text-center">重设用户 ${userName } (${userPhone }) 角色</div>
					<div class="form-group">
						<div class="col-sm-12">
							<select class="form-control" name="roleId">
								<option value="0">选择一个角色</option>
								<c:forEach items="${roles }" var="role">
									<option value="${role.roleId }" <c:if test="${currentRoleId==role.roleId }">selected</c:if>>${role.roleName }</option>
								</c:forEach>
							</select>
						</div>
					</div>
